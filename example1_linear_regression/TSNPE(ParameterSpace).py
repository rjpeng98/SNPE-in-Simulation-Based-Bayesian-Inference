...
"""
Check the performance of TSNPE (parameter space, with an example in Bayesian linear regression. (Compute Canada Version))
Author: Renjie Peng
Date: 2025-11-04
"""
import torch
...
# Process log(sigma2): (n,1)
log_sigma2_sim = torch.log(prior_sim['sigma2'])
# Concatenate to form parameters: (n, 3) → [beta0, beta1, log(sigma2)]
parameters_sim = torch.cat([beta_sim, log_sigma2_sim], dim=1)
...
mdn.eval()
with torch.no_grad():
    # Sample n samples from the estimated posterior for the observed Y_obs_stdzd
...
# ---------------------------
# Accumulate first-round data (NO WEIGHTS for TSNPE)
# ---------------------------
all_theta = [parameters_sim]
all_Y = [Y_sim]

# Initialize M_coverage_per_round list
M_coverage_per_round = []
...
for rnd in range(2, 31):
    ...
    with torch.no_grad():
        ...
        # Step 1: Sample from current estimated posterior
        ...
        # Step 2: Compute posterior log-probabilities in PARAMETER SPACE
        posterior_log_prob = mdn.log_prob(posterior_samples_std, Y_obs_stdzd.expand(n_proposal, -1))
        
        # Step 3: Determine quantile threshold for region M in PARAMETER SPACE
        mass = 0.999
        threshold = torch.quantile(posterior_log_prob, 1 - mass)
        print(f"  Threshold (1-{mass} quantile): {threshold:.4f}")

        # Calculate M coverage BEFORE oversampling (for efficiency)
        corr_samples_torch = torch.tensor(corr_samples_np, dtype=torch.float32)
        corr_samples_std = (corr_samples_torch - parameters_mean) / parameters_std
        corr_log_prob = mdn.log_prob(corr_samples_std, Y_obs_stdzd.expand(len(corr_samples_torch), -1))
        in_M = (corr_log_prob >= threshold).float().mean().item()
        M_coverage_per_round.append(in_M)
        print(f"  Proportion of true posterior in M: {in_M:.4f}")

        # Step 4: Oversample from prior and classify in PARAMETER SPACE
        n_oversample = n_proposal * 10
        print(f"  Oversampling: {n_oversample} samples from prior")
        
        proposal_samples_std_large = prior.sample((n_oversample,))
        proposal_samples_large = proposal_samples_std_large * parameters_std + parameters_mean

        # Step 5: Compute posterior log-probabilities for prior samples
        proposal_log_prob = mdn.log_prob(proposal_samples_std_large, Y_obs_stdzd.expand(n_oversample, -1))

        # Step 6: Classify in PARAMETER SPACE: M = {θ : p(θ|y_obs) >= threshold}
        mask_M_large = proposal_log_prob >= threshold

        idx_M = torch.where(mask_M_large)[0]
        
        print(f"  Target: {n_proposal} samples from M")
        ...
        print(f"  Estimated Z_M (for info): {Z_M:.4f}")

        # Step 7: Adaptive additional sampling if insufficient samples in M
        max_iterations = 5
        iteration = 0
        ...
        while len(idx_M) < n_proposal and iteration < max_iterations:
            ...
            additional_samples_std = prior.sample((n_additional,))
            additional_samples = additional_samples_std * parameters_std + parameters_mean
            
            # Classify additional samples in PARAMETER SPACE
            additional_log_prob = mdn.log_prob(additional_samples_std, Y_obs_stdzd.expand(n_additional, -1))
            mask_M_add = additional_log_prob >= threshold
            
            # Combine with existing samples
            proposal_samples_std_large = torch.cat([proposal_samples_std_large, additional_samples_std], dim=0)
        ...
        # Step 8: Sample ONLY from M (TSNPE truncation - no M^c samples!)
        if len(idx_M) >= n_proposal:
            selected_M = idx_M[torch.randperm(len(idx_M))[:n_proposal]]
            print(f"  ✓ Selected {n_proposal} UNIQUE samples from M")
        else:
        ...
        # Simulate data for selected parameters
        beta_prop = proposal_samples[:, :2]
        sigma2_prop = torch.exp(proposal_samples[:, 2]).unsqueeze(1)
        Y_prop = simulator(beta_prop.unsqueeze(2), sigma2_prop, m).squeeze(1)
        
        print(f"  All {n_proposal} samples are from region M (truncated)")
        
        # Add to accumulated data pool (NO WEIGHTS for TSNPE!)
    ...
    # Step 9: Train MDN WITHOUT importance weighting (standard NLL loss)
    best_state = None
    patience_counter = 0
    best_val_loss = float('inf')
    ...
    # Save M coverage
    np.savetxt(
        os.path.join(OUTDIR, f"M_coverage_per_round_seed{SEED}.csv"),
        M_coverage_per_round,
        delimiter=",",
        header="M_Coverage",
        comments=""
