...
"""
Example 1 (linear regression) with NPE
Author: Renjie Peng
Date: 2025-11-26
"""
import torch
...
for rnd in range(2, 31):
    ...
    with torch.no_grad():
        ...
        additional_samples_std = prior.sample((n_proposal,))
        additional_samples = additional_samples_std * parameters_std + parameters_mean

        # Simulate data for these additional samples
        beta_new = additional_samples[:, :2]
        sigma2_new = torch.exp(additional_samples[:, 2]).unsqueeze(1)
        Y_additional = simulator(beta_new.unsqueeze(2), sigma2_new, m).squeeze(1)
        
        all_theta.append(additional_samples)
