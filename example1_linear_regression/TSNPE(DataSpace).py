...
for rnd in range(2, 31):
    ...
    with torch.no_grad():
        ...
        beta_post = posterior_samples[:, :2]
        sigma2_post = torch.exp(posterior_samples[:, 2]).unsqueeze(1)
        Y_post = simulator(beta_post.unsqueeze(2), sigma2_post, m).squeeze(1)
        ...
        w_list = w_list / w_list.sum()  # Normalize weights
        ...
        beta_prop_large = proposal_samples_large[:, :2]
        sigma2_prop_large = torch.exp(proposal_samples_large[:, 2]).unsqueeze(1)
        Y_prop_large = simulator(beta_prop_large.unsqueeze(2), sigma2_prop_large, m).squeeze(1)
        ...
        while len(idx_M) < n_proposal and iteration < max_iterations:
            ...
            beta_add = additional_samples[:, :2]
            sigma2_add = torch.exp(additional_samples[:, 2]).unsqueeze(1)
            Y_add = simulator(beta_add.unsqueeze(2), sigma2_add, m).squeeze(1)
        ...
        # Step 7: Sample ONLY from M (TSNPE truncation - no M^c samples!)
...
plt.plot(range(1, 31), acc_per_round, marker='o')
plt.xlabel("Round")
