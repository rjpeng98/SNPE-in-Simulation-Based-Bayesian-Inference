...
# Define the SNPE-B loss without kernel density estimation
def snpe_b_loss(mdn, batch_Y, batch_theta, batch_weights):
    log_prob = mdn.log_prob(batch_theta, batch_Y).unsqueeze(1)
    loss = - torch.mean(log_prob * batch_weights, dim = 0,  keepdim=True)
    return(loss)
...
all_Y = [Y_sim]
all_weights = [torch.ones(n, device=parameters_sim.device, dtype=parameters_sim.dtype).unsqueeze(1)]


# Expand Y_obs_stdzd to match the number of samples
Y_obs_stdzd_expanded = Y_obs_stdzd.expand(n, -1)  # This will repeat Y_obs_stdzd n_samples times
...
for rnd in range(2, 31):
    ...
    with torch.no_grad():
        ...
        p_tilde = torch.exp(mdn.log_prob(samples, Y_obs_stdzd_expanded).unsqueeze(1))
        samples = samples.detach()
    # Unstandardize: original = std * sample + mean
    samples_unstd = samples * parameters_std + parameters_mean  # broadcasting over (n, 3)
    p_0 = ExactPriorStandardized(parameters_mean, parameters_std, m0, M0, a0, b0).p_0(samples_unstd).unsqueeze(1)
    weights_r = p_0/p_tilde
    ...
    weights_all = torch.cat(all_weights, dim = 0)
    ...
    weights_train, weights_val = weights_all[idx[:int(0.9*n_total)]], weights_all[idx[int(0.9*n_total):]]
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(theta_train), batch_size):
            ...
            batch_theta, batch_Y, batch_weights = theta_train[idx], Y_train[idx], weights_train[idx]

            optimizer.zero_grad()
            # Use updated mean/std for prior_std
            
            loss = snpe_b_loss(mdn, batch_Y, batch_theta, batch_weights)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(mdn.parameters(), max_norm=clip_max_norm)
            optimizer.step()
        ...
        mdn.eval()
        with torch.no_grad():
            val_loss = snpe_b_loss(mdn, Y_val, theta_val, weights_val).item()
