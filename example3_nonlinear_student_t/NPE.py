...
"""
Example 3 (nonlinear regression, Student-t errors) with NPE
Author: Renjie Peng
Date: 2026-1-19
"""
import torch
...
for rnd in range(2, 31):
    ...
    theta_all_std = (theta_all - parameters_mean) /torch.clamp(parameters_std, min=eps)
    Y_all_std = (Y_all - Y_mean) / torch.clamp(Y_std, min=eps)
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}: Avg Train Loss = {avg_train_loss:.4f}, Val Loss = {val_loss:.4f}, LR = {current_lr:.6f}")
    ...
    mdn.eval()
    with torch.no_grad():
        logits, means, precisions, *_ = mdn.get_mixture_components(Y_obs_stdzd)
