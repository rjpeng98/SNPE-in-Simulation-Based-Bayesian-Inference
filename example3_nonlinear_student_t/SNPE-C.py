...
"""
Example 3 (nonlinear regression, Student-t errors) with SNPE-C
Author: Renjie Peng
Date: 2026-2-2
"""
import torch
...
##########################
# APT Loss Function for SNPE-C (Atomic Proposal)
##########################
...
def apt_loss_batch(mdn, thetas_batch, Y_batch, prior_std, num_atoms=10):
    ...
    This implements the contrastive loss where each true parameter θᵢ is compared
    against (num_atoms - 1) randomly selected parameters from the batch, using
    the prior p₀ as the reference distribution.
    
    Args:
        mdn: Mixture density network
        thetas_batch: (B, 5) tensor of parameters in STANDARDIZED unconstrained space
        Y_batch: (B, m) tensor of observations
        prior_std: StandardizedPrior object with log_prob method
        num_atoms: Number of atomic candidates for each sample (including the true one)
    
    Returns:
        loss: Scalar tensor of the average APT loss over the batch
    """
    B = thetas_batch.size(0)
    device = thetas_batch.device
    
    # Check batch size is sufficient
    if B < num_atoms:
        raise ValueError(f"Batch size {B} must be >= num_atoms {num_atoms}")
    
    loss_vec = []

    for i in range(B):
        # Get the observation and true parameter for sample i
        x_i = Y_batch[i:i+1]  # (1, m)
        theta_i = thetas_batch[i:i+1]  # (1, 5)

        # Sample (num_atoms - 1) other candidates from the batch (excluding i)
        all_indices = torch.arange(B, device=device)
        other_indices = all_indices[all_indices != i]
        
        # Randomly select (num_atoms - 1) candidates
        perm = torch.randperm(len(other_indices), device=device)
        chosen_indices = other_indices[perm[:num_atoms - 1]]
        
        # Combine chosen candidates with the true parameter (put true parameter last)
        candidate_indices = torch.cat([chosen_indices, torch.tensor([i], device=device)])
        theta_candidates = thetas_batch[candidate_indices]  # (num_atoms, 5)
        
        # Compute log prior for all candidates
        log_p0 = prior_std.log_prob(theta_candidates)  # (num_atoms,)
        
        # Compute log probability from MDN for all candidates given x_i
        x_expanded = x_i.expand(num_atoms, -1)  # (num_atoms, m)
        log_q = mdn.log_prob(theta_candidates, x_expanded)  # (num_atoms,)
        
        # Compute APT loss: -log[q(θᵢ|xᵢ)/p₀(θᵢ)] + log[Σⱼ q(θⱼ|xᵢ)/p₀(θⱼ)]
        # This is equivalent to: -[log q(θᵢ|xᵢ) - log p₀(θᵢ)] + logsumexp[log q(θⱼ|xᵢ) - log p₀(θⱼ)]
        ratio = log_q - log_p0  # (num_atoms,)
        
        # The true parameter is at position (num_atoms - 1)
        loss_i = -ratio[-1] + torch.logsumexp(ratio, dim=0)
        loss_vec.append(loss_i)

    return torch.stack(loss_vec).mean()

print("✅ APT loss function defined for SNPE-C")
