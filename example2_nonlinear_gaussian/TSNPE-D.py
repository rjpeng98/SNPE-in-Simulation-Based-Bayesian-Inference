...
"""
Example 2 (nonlinear regression, Gaussian errors) with TSNPE-D
Author: Renjie Peng
Date: 2026-1-19
"""
import torch
...
##########################
# Compare Y_sim (direct prior) and Y_obs Marginals in Original Space
##########################

# Use the existing Y_sim from first round (already simulated from prior)
# Convert to numpy for plotting (original scale)
Y_sim_np = Y_sim.detach().cpu().numpy()  # shape: (n, m)
Y_obs_np = Y_obs.detach().cpu().numpy()  # shape: (m,)

# Create KDE plots for each design point
...
for i in range(m):
    ...
    # Plot KDE for Y_sim (first round prior samples) at design point i (original scale)
    sns.kdeplot(Y_sim_np[:, i], ax=ax, label='Y_prior', color='green', fill=True, alpha=0.3)
...
plt.tight_layout()
plot_filename = os.path.join(OUTDIR, f"Y_comparison_proposed_vs_obs_round1_seed{SEED}.png")
