...
print("Prior density function defined for TSNPE")
...
for rnd in range(2, 31):
    ...
    with torch.no_grad():
        prior = PriorStandardized(parameters_mean, parameters_std)
        n_proposal = n
        ...
        corr_samples_torch = torch.tensor(stan_unconstrained_np, dtype=torch.float32)
        ...
        proposal_samples_constrained = sample_prior(n_oversample)
        proposal_samples_large = transform_to_unconstrained(proposal_samples_constrained, lower=0.0, upper=10.0)
        ...
        # Extract selected samples (all from M) in unconstrained space
        proposal_samples = proposal_samples_large[selected_M]
        
        # Transform back to constrained space for simulation
        proposal_samples_constrained = transform_to_constrained(proposal_samples, lower=0.0, upper=10.0)
        
        # Simulate data for selected parameters
        Y_prop = simulator(proposal_samples_constrained, m)
    ...
    # Step 10: Train MDN WITHOUT importance weighting (standard NLL loss)
    best_state = None
    patience_counter = 0
    best_val_loss = float('inf')
    ...
    # Step 11: Sample from posterior and evaluate
    with torch.no_grad():
        # Sample from MDN (in standardized unconstrained space)
